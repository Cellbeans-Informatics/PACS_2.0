...
const SidePanel = ({
  ...
  const YourButtonComponent = ({ onClick, label }) => <button onClick={onClick}>{label}</button>;
      ...
      <>
        <div
          className={classnames(
            'bg-g flex h-[28px] w-full cursor-pointer items-center rounded-md',
  ...
  const getOpenStateComponent = () => {
    return (
      <div
        style={{ backgroundColor: '#313131' }}
        className="bg-primary-dark flex rounded-t pt-1.5 pb-[2px]"
      >
        {getCloseIcon()}
        ...
        {side === 'right' && (
          <>
            {/* <YourButtonComponent
              onClick={yourButtonClickHandler}
              label="Your Button"
            /> */}
            <div className="your-new-div">
              <button
                style={{
    ...
    >
      {panelOpen ? (
        ...
        <React.Fragment>
          ...
          <div className="partition-area">Report </div>
        </React.Fragment>
      )}
    </div>
  );
